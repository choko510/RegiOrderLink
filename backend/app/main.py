...
from .models import Base as ModelsBase, Table, Menu, Order, OrderItem
from .routers import tables, menus, orders  # 後でインポート
...
def init_data(db: Session = Depends(get_db)):
    ...
    menus_data = [
        ...
        {"name": "サラダ", "price": 300, "category": "サイド"},
        {"name": "ドリンク", "price": 200, "category": "ドリンク"}
    ]
    for m in menus_data:
        db_menu = Menu(**m)
        db.add(db_menu)
